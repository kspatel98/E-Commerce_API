    ...
    try {
        if (Array.isArray(req.body)) {
            ...
            const products = req.body.map(product => {
                if (!product.name || !product.price || !product.description || !product.brand || !product.category || !product.images || !product.merchantName || !product.merchantLocation) {
                    return res.status(400).json({ message: "Name, price, description, brand, category, images, isFeatured, merchantName, and merchantLocation are required fields." });
                }
                return new Product({
                    name: product.name,
                    price: product.price,
            ...
            if (!req.body.name || !req.body.price || !req.body.description || !req.body.brand || !req.body.category || !req.body.images || !req.body.merchantName || !req.body.merchantLocation) {
                return res.status(400).json({ message: "Name, price, description, brand, category, images, isFeatured, merchantName, and merchantLocation are required fields." });
            }
...
router.patch('/:id', getProducts, async (req, res) => {
    ...
    try {
        if (!req.body.name && !req.body.price && !req.body.description && !req.body.brand && !req.body.category && !req.body.images && !req.body.isFeatured && !req.body.merchantName && !req.body.merchantLocation) {
